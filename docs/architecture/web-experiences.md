...
| URL (dev) | `/` , `/experts` , `/pricing` , `/how-it-works` , `/for-experts` , `/about` , `/legal/*` , `/blog/*` (later) | `/login` , `/register` , `/account` , `/onboarding/*` , `/expert/*` , `/student/*` , `/dashboard` , `/messages/*` , `/orders/*` , `/requests/*` , `/settings` (functional areas arrive with their phases) | `/portal` (dashboard = analytics), `/portal/moderation` , `/portal/disputes` , `/portal/finance` , `/portal/audit` , `/portal/users` , `/portal/config` — live since Phase 10; expert-applications + order triage stay Django admin (deep-linked) |
...
4. **Django admin and the portal share the ops role since Phase 10** (ADR-0010): the portal owns dashboards/queues/audit/config (surfaces Django admin cannot express); Django admin keeps triage/approvals/resolution (deep-linked from the portal).
