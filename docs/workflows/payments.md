...
- **Refunds** (`payments.services.issue_refund`): staff-only, full or partial, validated against the remaining refundable balance; proportional commission/expert-credit reversal entries; `Payment.status` moves to `refunded|partially_refunded`. Since Phase 9 this is also the **only money path for dispute resolutions** (`Refund.reason="dispute_resolution"`) — dispute code never writes ledger rows directly (docs/workflows/disputes.md).
...
- **Dispute freeze (Phase 9, implemented):** `schedule_payout` skips orders with `order.has_open_dispute`; `settle_payout` re-reads the order `select_for_update` and raises `payout_frozen` while a dispute is open; `payout_sweeper` excludes disputed orders. Both open_dispute and settle_payout lock the ORDER row first, so dispute-vs-settlement races serialize (tested). On resolution: full refund voids a still-unsettled payout (`mark_payout_failed`), partial/split **adjust** it to the remaining expert credit (audited `payout.adjusted_dispute`), release unfreezes it as-is.
